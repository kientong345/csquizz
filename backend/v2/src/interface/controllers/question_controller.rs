...
use crate::{
    application::services::question_service::QuestionService,
    interface::dto::question_dto::{QuestionDto, CreateQuestionDto, UpdateQuestionDto, ListQuestionsQuery},
...
pub async fn list_questions(
    ...
    match app_state.question_service.list_questions(query).await {
        Ok((questions, total_items, total_pages)) => {
            let question_dtos: Vec<QuestionDto> = questions.into_iter().map(QuestionDto::from).collect();
...
pub async fn get_question(
    ...
    match app_state.question_service.get_question_by_id(question_id).await {
...
pub async fn update_question(
    ...
    match app_state.question_service.update_question(question_id, dto).await {
...
pub async fn delete_question(
    ...
    match app_state.question_service.delete_question(question_id).await {
